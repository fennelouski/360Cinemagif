...
@property (nonatomic, strong) UIButton *exportButton, *recordButton;
...
- (CGRect)totalFrameFrame {
    ...
    if (exporting) {
        frame.size.width = videoSize.width * 0.5f;//frame.size.height * 2.0f;
        frame.size.height = videoSize.height * 0.5f;
    }
...
- (void)start {
    ...
    if ([self.AVSession canSetSessionPreset:AVCaptureSessionPreset352x288]) {
...
- (UILabel *)lbl_pitch {
    if (!_lbl_pitch) {
        _lbl_pitch = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 20.0f, 60.0f, 44.0f)];
...
- (UILabel *)lbl_yaw {
    if (!_lbl_yaw) {
        _lbl_yaw = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 64.0f, 60.0f, 44.0f)];
...
- (UILabel *)lbl_roll {
    if (!_lbl_roll) {
        _lbl_roll = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 108.0f, 60.0f, 44.0f)];
...
- (UILabel *)lbl_isvalid {
    if (!_lbl_isvalid) {
        _lbl_isvalid = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 152.0f, 60.0f, 44.0f)];
...
- (void)saveNextFrame:(NSNumber *)frameNumber {
    int numberOfImagesPerBatch = 300;
    ...
    if (frameNumber.intValue < self.images.count && !receivedMemoryWarning && finalImageSequence.count < numberOfImagesPerBatch) {
        ...
        NSLog(@"Sequences: %zd", reversed.count);
...
- (void)convertImagesToMovie:(NSArray *)imageSequence {
    ...
    [VRExportVideo saveMovieToLibrary:imageSequence];
}
...
//    NSLog(@"Nothing will happen here either...I changed the same parameters as at the other place");
    ...
    NSLog(@"Image is: %g x %g", image.size.width, image.size.height);
